                                    ...
                                    {slotAppointments.map((apt) => {
                                        ...
                                        return (
                                            <Card
                                                ...
                                                className={cn(
                                                    "flex items-center justify-between p-4 bg-card border shadow-sm hover:shadow-md transition-all cursor-pointer border-l-4",
                                                    config.borderClass,
                                                    isActive ? "ring-2 ring-primary/50 border-primary" : "border-border/60",
                                                    apt.status === 'cancelled' && "opacity-60 saturate-50"
                                                )}
                                            >
                                                <div className="flex items-center space-x-4 min-w-0 flex-1">
                                                    <div className={cn(
                                                        "h-10 w-10 rounded-full flex items-center justify-center shrink-0 border border-border/50 shadow-inner",
                                                        isActive ? "bg-primary/10 text-primary" : "bg-muted/50 text-muted-foreground"
                                                    )}>
                                                        {apt.status === 'fulfilled' ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <User className="w-5 h-5" />}
                                                    </div>
                                                    <div className="flex flex-col min-w-0">
                                                        ...
                                                        <div className="text-xs text-muted-foreground flex items-center gap-2 mt-1">
                                                            <span className="font-medium px-1.5 py-0.5 rounded bg-muted/60">
                                                ...
                                                <div className="flex items-center gap-3 shrink-0 ml-4">
                                                    <Badge 
                                                        ...
                                                        className={cn(
                                                            "h-6 px-2 text-[10px] uppercase font-bold tracking-wider",
